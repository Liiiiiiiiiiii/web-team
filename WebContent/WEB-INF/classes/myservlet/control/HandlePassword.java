...
public class HandlePassword extends HttpServlet{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		...
		if (ok == true){
			continueWord(request, response);
		}
	}
	
	public void continueWord(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		HttpSession session = request.getSession(true);
		Login login = (Login)session.getAttribute("login");
		Connection con = null;
		String logname = login.getLogname();
		Password passwordBean = new Password();
		request.setAttribute("passowrd", passwordBean);
		...
		try{
			...
			ResultSet rs = sql.executeQuery("SELECT * FROM value where logname = "
					+ " '" + logname + "' And password = '" + oldPassword + "'");
			if (rs.next()){
				String updateString = "UPDATE value SET password = '" + newPassword + "' where logname = '" + logname + "'";
				...
				if (m == 1){
					...
					passwordBean.setBackNews("更新密码失败");
			}
			else 
				passwordBean.setBackNews("更新密码失败");
		}
