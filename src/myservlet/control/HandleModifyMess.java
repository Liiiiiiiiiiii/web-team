...
public class HandleModifyMess extends HttpServlet{
	...
	public void continueDoPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		...
		String backNews ="";
		try{
			String uri = "jdbc:sqlserver://127.0.0.1/Team?" + "user = sa&password = 7gcgdi66 &characterEncoding = gb2312";
			con = DriverManager.getConnection(uri);
			String updateCondition = "UPDATE value SET phone ='" + phone + "',"
					+ "email = '" + email + "', message = '" +message+"'WHERE logname = '"+logname+"'";
			...
			if (m == 1){
				...
				modify.setMessage(message);
			}
			else {
				backNews = "你的信息填写不完整或信息中有非法字符";
				modify.setBackNews(backNews);
			}
			con.close();
		}
